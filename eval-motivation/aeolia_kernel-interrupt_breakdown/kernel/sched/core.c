 ...
 */
extern struct task_struct *g_trace_task;
extern int g_trace_cpu;
...
static void __sched notrace __schedule(int sched_mode)
{
	...
	if(smp_processor_id() == g_trace_cpu && current == g_trace_task && next == idle_task(smp_processor_id())) {
		g_trace_task->thread.breakdown_timestamps[2] = rdtsc();
	}
	
	if(smp_processor_id() == g_trace_cpu && next == g_trace_task)
		g_trace_task->thread.breakdown_timestamps[8] = rdtsc();
picked:
