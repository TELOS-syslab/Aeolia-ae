			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	if(current == g_trace_task)
		current->thread.breakdown_timestamps[1] = rdtsc();
	spin_unlock(&nvmeq->sq_lock);
	return BLK_STS_OK;
}
...
{
	if(smp_processor_id() == g_trace_cpu) {
		g_trace_task->thread.breakdown_timestamps[3] = rdtsc();
	}
	...
	if (nvme_poll_cq(nvmeq, &iob)) {
		...
		if (smp_processor_id() == g_trace_cpu) {
			g_trace_task->thread.breakdown_timestamps[7] = rdtsc();
		}
		return IRQ_HANDLED;
	}
	return IRQ_NONE;
