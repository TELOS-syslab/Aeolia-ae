...
struct task_struct *g_trace_task = NULL;
EXPORT_SYMBOL(g_trace_task);
					 ...
					 unsigned flags)
{
	...
	if (nr_tw_prev >= nr_wait)
		return;
	g_trace_task->thread.breakdown_timestamps[4] = rdtsc();
	wake_up_state(ctx->submitter_task, TASK_INTERRUPTIBLE);
	g_trace_task->thread.breakdown_timestamps[5] = rdtsc();
}
...
static int io_issue_sqe(struct io_kiocb *req, unsigned int issue_flags)
{
	...
	current->thread.breakdown_timestamps[11] = rdtsc();
			  ...
			  struct ext_arg *ext_arg)
{
	...
	if(current == g_trace_task) {
		current->thread.breakdown_timestamps[9] = rdtsc();
	}
		...
		size_t, argsz)
{
	...
	struct file *file;
	long ret;
	if(!g_trace_task && to_submit) {
		g_trace_task = current;
		g_trace_cpu = smp_processor_id();
		g_trace_task->thread.breakdown_timestamps[0] = rdtsc();
	}
...
iopoll_locked:
		...
		if (flags & IORING_ENTER_GETEVENTS && !to_submit) {
			current->thread.breakdown_timestamps[10] = rdtsc();
			pr_info("[BREAKDOWN_METRICS] : %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu\n",
				current->thread.breakdown_timestamps[0],
				current->thread.breakdown_timestamps[1],
				current->thread.breakdown_timestamps[2],
				current->thread.breakdown_timestamps[3],
				current->thread.breakdown_timestamps[4],
				current->thread.breakdown_timestamps[5],
				current->thread.breakdown_timestamps[6],
				// current->thread.breakdown_timestamps[7],
				current->thread.breakdown_timestamps[8],
				current->thread.breakdown_timestamps[9],
				current->thread.breakdown_timestamps[10],
				current->thread.breakdown_timestamps[11],
				current->thread.breakdown_timestamps[12],
				current->thread.breakdown_timestamps[13],
				current->thread.breakdown_timestamps[14],
				current->thread.breakdown_timestamps[15],
				current->thread.breakdown_timestamps[16],
				current->thread.breakdown_timestamps[7]);
			memset(current->thread.breakdown_timestamps, 0,
			       sizeof(current->thread.breakdown_timestamps));
			g_trace_task = NULL;
			g_trace_cpu = -1;
		}
		if (!ret) {
