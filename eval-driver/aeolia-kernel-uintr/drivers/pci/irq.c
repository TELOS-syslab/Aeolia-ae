		...
		irq_handler_t thread_fn, void *dev_id, const char *fmt, ...)
{
	...
	uint32_t irq = pci_irq_vector(dev, nr);
	struct irq_data *irq_data = irq_get_irq_data(irq);
	pr_info("[pci_request_irq] devname : %s irq_num : %d, msi: %d, cpuid: %d, cpu_ndst: %d, affinity: %*pbl\n",
		devname, irq, get_vec_by_msi_irq(irq), smp_processor_id(),
		apic->cpu_present_to_apicid(smp_processor_id()),
		cpumask_pr_args(irq_data->common->affinity));
	if (ret)
		kfree(devname);
	return ret;
}
