...
static void setup_uintr(struct cpuinfo_x86 *c)
{
	...
	if (!cpu_feature_enabled(X86_FEATURE_UINTR)) {
		pr_info_once("UINTR not enabled in cpu\n Dis : %d stchas %d\n", 
			DISABLED_MASK_BIT_SET(X86_FEATURE_UINTR),
			static_cpu_has(X86_FEATURE_UINTR));
		goto disable_uintr;
	}
	pr_info_once("UINTR enabled in cpu :%d\n", cpu_has(c, X86_FEATURE_UINTR));
	/* checks the current processor's cpuid bits: */
	if (!cpu_has(c, X86_FEATURE_UINTR)) {
		pr_info_once("UINTR not enabled in cpu - has\n");
		goto disable_uintr;
	}
	if (!cpu_has_xfeatures(XFEATURE_MASK_UINTR, NULL)) {
		pr_info_once("UINTR XSAVE support missing");
