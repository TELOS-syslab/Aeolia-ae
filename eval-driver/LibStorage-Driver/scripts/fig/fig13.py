...
# For 4-core co-run
csv_name = "06_corun_io_io_4_cores"
input_csv = os.path.join(data_dir, csv_name + ".csv")
...
set yrange [0:0.8]
set ytic 0.2
set ytic add ('4' 0.6, '50' 0.8)

set arrow from 0,0.5 to 4.5,0.5 nohead lc rgb "white" lw 2.2 front
set arrow from -1.1,0.5 to -0.9,0.5 nohead lc rgb "white" lw 2.2 front

set arrow from -1.06,0.5 to -0.94,0.516 nohead lc rgb "black" lw 0.5 front
set arrow from -1.06,0.486 to -0.94,0.502 nohead lc rgb "black" lw 0.5 front
"""
plot_script += f'plot "{data_files[0]}" \\'
base = baselines[0]
plot_script += base_bar.format(
    entry=f"{map_to_interval(base.id, 4000, 50000, 0.6, 0.8)}:xtic(4)",
    color=base.color,
...
for base in baselines[1:]:
    curve = base_bar_notitle.format(
        entry=map_to_interval(base.id, 4000, 50000, 0.6, 0.8),
        color=base.color,
        pattern=base.pattern,
        title=base.title,
    )
...
# Second subplot
plot_script += base_next_plot.format(title=r"(b) single core: total throughput")
plot_script += """
set xtics noenhanced
set ylabel "Total Throughput (GB/s)" offset 0.5,0
set yrange [0:8]
set ytic 2
unset arrow
"""
...
set yrange [0:0.405]
set ytic 0.1
set ytic add ('4' 0.3, '8' 0.4)

set arrow from 2.5,0.25 to 4.5,0.25 nohead lc rgb "white" lw 2.2 front
set arrow from -1.1,0.25 to -0.9,0.25 nohead lc rgb "white" lw 2.2 front

set arrow from -1.06,0.25 to -0.94,0.258 nohead lc rgb "black" lw 0.5 front
set arrow from -1.06,0.243 to -0.94,0.251 nohead lc rgb "black" lw 0.5 front
"""
plot_script += f'plot "{data_files[1]}" \\'
base = baselines[0]
plot_script += base_bar_notitle.format(
    entry=f"{map_to_interval(base.id, 4000, 8000, 0.3, 0.4)}:xtic(4)",
    color=base.color,
    pattern=base.pattern,
    title=base.title,
)
for base in baselines[1:]:
    curve = base_bar.format(
        entry=map_to_interval(base.id, 4000, 8000, 0.3, 0.4),
        color=base.color,
        pattern=base.pattern,
        title=base.title,
    )
...
# Fourth subplot
plot_script += base_next_plot.format(title=r"(d) 4 cores: total throughput")
plot_script += """
set xtics noenhanced
set ylabel "Total Throughput (GB/s)" offset 0.5,0
set yrange [0:8]
