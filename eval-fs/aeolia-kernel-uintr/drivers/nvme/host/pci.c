...
static int nvme_setup_irqs(struct nvme_dev *dev, unsigned int nr_io_queues)
{
	...
	// 				      &affd);
	int ret = pci_alloc_irq_vectors(pdev, 1, 128, PCI_IRQ_ALL_TYPES);
	pr_info("[NVME]nvme alloc reqs : %d", ret);
	return irq_queues;
}
...
static unsigned int nvme_max_io_queues(struct nvme_dev *dev)
{
	...
	if (dev->ctrl.quirks & NVME_QUIRK_SHARED_TAGS)
		return 1;
	return LIBSTORAGE_ALLOW_KERNEL_IO_QUEUES + dev->nr_write_queues +
	       dev->nr_poll_queues;
}
...
static int nvme_setup_io_queues(struct nvme_dev *dev)
{
 ...
 retry:
	...
	// BUG: Hardcoded alloc irqs
	result = nvme_setup_irqs(dev, nr_io_queues);
	pr_info("[NVME] irq 16 : %d, irq 32 : %d", pci_irq_vector(pdev, 16),
		pci_irq_vector(pdev, 32));
	if (result <= 0) {
		result = -EIO;
		goto out_unlock;
	}
