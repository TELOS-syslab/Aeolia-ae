...
int create_qp(ls_queue_args *args, ls_nvme_dev *nvme, ls_nvme_qp *qp, uint32_t in_trust,
	      uint32_t out_trust)
{
	int ret = 0;
	char path[PATH_SIZE];
	...
	qp->in_trust = in_trust;
	qp->out_trust = out_trust;
	LOG_INFO("Create QP %d, in_trust : %d, out_trust : %d", qp->qid,
		 qp->in_trust, qp->out_trust);
	// CQ-----------------------------
	snprintf(path, PATH_SIZE, "/dev/" LIBSTORAGE_CQ_PATH_FORMAT, nvme->id,
		 qp->qid);
	qp->cq_fd = open(path, O_RDWR);
	if (qp->cq_fd < 0) {
		LOG_WARN("Failed to open cq\n");
		return qp->cq_fd;
	}
...
int create_dma_buffer(ls_nvme_dev *nvme, dma_buffer *buf, uint32_t size)
{
	...
	uint64_t *pa;
	tot_size += size;
	// LOG_WARN("Allocating %d bytes, total size: %lu", size, tot_size);

	ret = alloc_dma_buffer(nvme->dma_pool, size, &va, &pa);
	if (ret < 0) {
		LOG_WARN("Failed to alloc dma buffer, tot_size:%ld, dma_pool size: %ld\n", tot_size, nvme->dma_pool->size);
		abort();
		return ret;
	}
