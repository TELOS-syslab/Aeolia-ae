...
inline int ls_qpair_process_completions(ls_nvme_qp *qp, int maxn)
{
	...
	while (1) {
		cqe = (volatile struct nvme_completion *)&(qp->cqes[qp->cq_head]);
		// cqe_tmp = *cqe;
		if (cqe->status >> 1) {
			printf("CQE status : %x\n", cqe->status);
			assert(0);
		}
		if ((*(volatile __le16*)(&cqe->status) & 1) != qp->cq_phase) {
			// if (!g_interrupt_coalescing || exit ||
			//     qp->interrupt == LS_INTERRPUT_POLLING) {
			// 	return 0;
			// }
			// exit = 1;
			// qp->nvme->upid->puir = 2;
		...
		found++;
		__sync_synchronize();
		iod = &qp->iods[*(volatile __u16*)(&cqe->command_id)];
		int lst = nvme_complete_iod(qp, iod);
		u32 tmp = qp->cq_head + 1;
		// exit = 0;
		if (tmp == qp->depth) {
			qp->cq_head = 0;
			qp->cq_phase ^= 1;
		...
		g_complete++;
		// if (g_interrupt_coalescing && lst == 0) {
		// 	exit = 1;
		// 	qp->nvme->upid->puir = 2;
		// }
	}
	if (found){writel(qp->cq_head, qp->cq_db);}
	return ret;
}
static inline int nvme_complete_iod(ls_nvme_qp *qp, io_desc *iod)
{
...
static int ls_submit_req(ls_nvme_qp *qp, io_request *req)
{
	...
	LOG_DEBUG(
		...
		iod_id, req->opcode, qp->sq_tail, 0ul);
		//nvme->upid->puir);
	// getchar();
	return 0;
}
		    ...
		    dma_buffer *buf, void (*callback)(void *), void *data)
{
	...
	LOG_DEBUG("qp :%p, Prepared IO Request : %p, child : %d",qp, req,
		  req->num_children);
	ret = ls_submit_req(qp, req);
	// _stui();
	return ret;
