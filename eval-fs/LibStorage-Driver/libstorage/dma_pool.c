...
int create_dma_pool(struct dma_pool *pool, uint64_t size, int instance_id)
{
	...
	if (size % HUGE_PAGE_SIZE != 0 || size > free_hpages * HUGE_PAGE_SIZE) {
		LOG_ERROR("Invalid size : %lu, free_hpage : %lu", size,
			  free_hpages);
		return -1;
	}
	...
	memset((void*)(pool->free_bitmap), 0, pool->bitmap_len * sizeof(uint64_t));
	pool->last_alloc_map = 0;
	pool->last_alloc_bit = 0;
	// for(int i=0; i < pool->bitmap_len; i++) {
	// 	printf("bitmap[%d]: %lx ", i, pool->free_bitmap[i]);
	// }
	// printf("\n");
		     ...
		     uint64_t **pa)
{
	uint32_t i, len;
	if (size % PAGE_SIZE != 0) {
		// LOG_WARN("size is not page aligned %d\n", size);
		return -1;
	}
	// TODO: resize the hugepage file.
	if (size > pool->size) {
		// LOG_WARN("size is larger than pool size %d\n", size);
		return -1;
	}
	len = size / PAGE_SIZE;
	...
	// fast path
	if (pool->last_alloc_map < pool->bitmap_len) {
		for (i = pool->last_alloc_map; i < pool->bitmap_len; i++) {
			if (pool->free_bitmap[i] == ~(uint64_t)0) {
				count = 0;
				continue;
			}
			for (int j = pool->last_alloc_bit; j < 64; j++) {
				if ((pool->free_bitmap[i] & (1ul << j)) == 0) {
					if (count == 0) {
						start = i * 64 + j;
					}
					count++;
					if (count == len) {
						pool->last_alloc_map = i;
						pool->last_alloc_bit = j + 1;
						goto found;
					}
				} else {
					count = 0;
	...
	// 	printf("bitmap[%d]: %lx ", i, pool->free_bitmap[i]);
	// }
	// LOG_WARN("\nno free page found bitmaplen: %ld, pagesize: %d\n" , pool->bitmap_len, size);
	// LOG_WARN("tot_len: %d\n", tot_len);
	return -1;
found:
	for (i = 0; i < len; i++) {
		// tot_len++;
		pool->free_bitmap[(start + i) / 64] |= 1ul << ((start + i) % 64);
	}
