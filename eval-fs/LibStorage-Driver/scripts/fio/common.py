...
fio_options = "--lat_percentiles=1 --clat_percentiles=0 --output={log_file} --output-format=json"
base_config = """
[global]
ioengine={ioengine}
rw={iotype}
...
base_bg_command = """
nice -n 19 taskset -c {cpus_allowed} {root_dir}/swaptions \
...
def run_fio_tests(eval_dir, pbar, engine, engine_name, filename, envs, engine_config, policy,
                  iotypes, iodepths, iosizes, num_threads, cpus_allowed, other_tests):
    ...
    for iotype, iodepth, iosize, numjobs in it.product(iotypes, iodepths, iosizes, num_threads):
        ...
        with tempfile.NamedTemporaryFile(mode="w+", suffix=".fio") as f:
            ...
            pbar.set_description(f"Running {engine}: iotype={iotype} iodepth={iodepth} iosize={iosize} numjobs={numjobs}")
...
def run_corun_with_comp_tests(eval_dir, pbar, engine, engine_name, filename, envs, engine_config,
                  iotypes, iodepths, iosizes, num_threads, cpus_allowed):
    result_dir = os.path.join(eval_dir, engine)
    os.makedirs(result_dir, exist_ok=True)
...
def fio_main(eval_file, engines, iotypes, iodepths, iosizes, num_threads, cpus_allowed, other_tests):
    if results_dir == "":
        print("Please source env.sh first!")
        sys.exit(1)
    ...
    total_tests = len(engines) * len(iotypes) * len(iodepths) * len(iosizes) * len(num_threads)
    with tqdm(total=total_tests, desc="Running FIO Tests", unit="test", ncols=100, colour="green") as pbar:
        for engine in engines:
            ...
            if engine == psync:
                ...
                subprocess.run(f"sudo PCI_ALLOWED={pcie_addr} bash {spdk_dir}/scripts/setup.sh config", shell=True, check=True)
            elif engine == io_uring:
                engine_config += "\ncpus_allowed_policy=shared"
                policy="shared"
                envs = {}
            elif engine == bypassd:
                # TODO: Bypassd configuration
                envs = {}
            ...
            # LibSched tests
            if engine == aeolia_sched:
                libsched = os.getenv("INTUS_LIBSCHED", "")
                libsched_command = f"sudo {libsched}"
                process = subprocess.Popen(libsched_command, shell=True)

                time.sleep(1)
                if process.poll() is not None:
                    raise RuntimeError(f"libsched failed to start. Return code: {process.returncode}")
                
                if other_tests == comp:
                    run_corun_with_comp_tests(eval_dir, pbar, engine, engine_name, filename, envs, engine_config,
                                iotypes, iodepths, iosizes, num_threads, cpus_allowed)
                else:
                    run_fio_tests(eval_dir, pbar, engine, engine_name, filename, envs, engine_config, policy,
                                iotypes, iodepths, iosizes, num_threads, cpus_allowed, other_tests)
            ...
            if engine == spdk:
                spdk_dir = os.getenv("INTUS_SPDK_DIR", "")
                subprocess.run(f"sudo bash {spdk_dir}/scripts/setup.sh reset", shell=True, check=True)
            elif aeolia in engine:
                subprocess.run(
                    ["sudo", "make", "clean"],
                    cwd=root_dir,
