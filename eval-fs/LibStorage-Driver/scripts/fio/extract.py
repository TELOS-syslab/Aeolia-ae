...
metrics = ["bw", "iops", "lat_avg", "lat_p99", "lat_p999"]
...
def extract_data_to_tables(data, engines, iotypes, iodepths, iosizes, num_threads, num_tasks):
    ...
    for engine, iotype, iodepth, iosize, numjobs, i in \
        ...
        for metric in metrics:
            tables[i][row_key][metric][engine] = io_data[metric]
...
def write_tables_to_csv(eval_name, tables, output_dir, engines):
    ...
    for i, table in tables.items():
        csv_file = os.path.join(output_dir, f"{eval_name}_{i}.csv")

        with open(csv_file, mode="w", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(headers)
            for row_key, row_data in table.items():
                row = list(row_key)
                row.extend(
