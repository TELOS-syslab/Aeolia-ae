...
base_preset = """\
call "common.gnuplot" "{width}in, {height}in"
set output "{output}"
...
def filter_csv_multicore(input_csv, output_csv, params, metrics):
    header = ["iotype", "iodepth", "iosize", "numcpu"] + metrics
...
def filter_csv_percore_tp(input_csv, output_csv, params, metrics):
    ...
    with open(input_csv, mode="r", encoding="utf-8") as infile, open(
        ...
        for row in reader:
            ...
            if valid == True:
                ...
                for col in metrics:
                    filtered_row[col] = str(float(filtered_row[col]) / float(4))
                writer.writerow(filtered_row)


def filter_csv_percore_tp_multicore(input_csv, output_csv, params, metrics):
    header = ["iotype", "iodepth", "iosize", "numcpu"] + metrics
    ...
    with open(input_csv, mode="r", encoding="utf-8") as infile, open(
        ...
        for row in reader:
            ...
            if valid == True:
                ...
                for col in metrics:
                    filtered_row[col] = str(
                        float(filtered_row[col]) / float(filtered_row["numcpu"])
                    )
                writer.writerow(filtered_row)
