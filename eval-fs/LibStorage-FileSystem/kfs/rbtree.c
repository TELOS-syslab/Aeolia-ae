...
int sufs_init_rangenode_cache(void) {
    sufs_range_node_cachep = kmem_cache_create("sufs_range_node_cache",
                                               sizeof(struct sufs_range_node),
...
int sufs_rbtree_insert_range_node(struct rb_root *tree,
    ...
    while (*temp) {
        curr = container_of(*temp, struct sufs_range_node, node);
        compVal =
            sufs_rbtree_compare_range_node(curr, new_node->range_low, type);
        parent = *temp;
        ...
        if (compVal == -1) {
            ...
            WARN_FS("%s: entry %lu - %lu already exists: "
                    "%lu - %lu\n",
