...
static inline struct sufs_range_node *sufs_alloc_range_node(void) {
    struct sufs_range_node *p;

    p = (struct sufs_range_node *)kmem_cache_zalloc(sufs_range_node_cachep,
                                                    GFP_NOFS);

    return p;
}

static inline void sufs_free_range_node(struct sufs_range_node *node) {
    kmem_cache_free(sufs_range_node_cachep, node);
}

static inline struct sufs_range_node *sufs_alloc_blocknode(void) {
    return sufs_alloc_range_node();
}

static inline void sufs_free_blocknode(struct sufs_range_node *node) {
    sufs_free_range_node(node);
}

static inline struct sufs_range_node *sufs_alloc_inode_node(void) {
    return sufs_alloc_range_node();
}

static inline void sufs_free_inode_node(struct sufs_range_node *node) {
    sufs_free_range_node(node);
}
...
int sufs_rbtree_find_range_node(struct rb_root *tree, unsigned long key,
                                enum node_type type,
                                struct sufs_range_node **ret_node);
