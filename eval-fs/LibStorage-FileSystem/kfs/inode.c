...
int sufs_alloc_inode_free_list(struct sufs_super_block *sb) {
    ...
    sb->inode_free_lists =
        kcalloc(sb->cpus, sizeof(struct sufs_inode_free_list), GFP_KERNEL);

    if (!sb->inode_free_lists) {
        WARN_FS("%s: Allocating inode maps failed.", __func__);
        return -ENOMEM;
    }
...
static void __sufs_init_inode_free_list(struct sufs_super_block *sb,
                                        ...
                                        int cpu) {
    int per_list_inode_pages = 0;

    per_list_inode_pages = FS_INODE_BITMAP_LOGICAL_BLOCKS / (sb->cpus);

    free_list->inode_start = per_list_inode_pages * cpu;
    free_list->inode_end = free_list->inode_start + per_list_inode_pages - 1;

    if (free_list->inode_start == 0) {
        free_list->inode_start = 1;
    }
}
...
static void sufs_inode_to_cpu(struct sufs_super_block *sb, int ino, int *cpu) {
    int per_list_inodes = 0;
    int cpu_tmp = 0;

    per_list_inodes = FS_INODE_BITMAP_LOGICAL_BLOCKS / (sb->cpus);
...
static int sufs_inode_find_free_slot(struct rb_root *tree, int range_low,
                                     int range_high,
                                     struct sufs_range_node **prev,
    ...
    if (ret) {
        WARN_FS("%s ERROR: %d - %d already in free list\n", __func__, range_low,
                range_high);
        return -EINVAL;
    }
    ...
    if (!ret_node) {
        ...
        *next = ret_node;
        ...
        WARN_FS("%s ERROR: %d - %d overlaps with existing "
                "node %ld - %ld\n",
...
int sufs_free_inodes(struct sufs_super_block *sb, int ino, int num) {
    ...
    if (ino < free_list->inode_start || ino + num > free_list->inode_end + 1) {
        WARN_FS("free inodes %d to %d, free list addr: %lx, "
                "start %d, end %d\n",
                ino, ino + num - 1, (unsigned long)free_list,
                free_list->inode_start, free_list->inode_end);
    ...
    if (ret) {
        WARN_FS("%s: find free slot fail: %d\n", __func__, ret);
        goto out;
    }
...
static int not_enough_inodes(struct sufs_inode_free_list *free_list,
     ...
     */
    if (!first || !last ||
        free_list->num_free_inodes / free_list->num_inode_node < num_inodes) {
        WARN_FS("%s: num_free_inodes=%d; num_inodes=%d; "
                "first=0x%p; last=0x%p",
                __func__, free_list->num_free_inodes, num_inodes, first, last);
        return 1;
    }
...
static int
sufs_alloc_inodes_in_free_list(struct sufs_super_block *sb,
                               struct sufs_inode_free_list *free_list,
                               int num_inodes, int *new_inode_nr) {
    struct rb_root *tree = NULL;
    ...
    if (!free_list->first_node || free_list->num_free_inodes == 0) {
        WARN_FS("%s: Can't alloc. inode_free_list->first_node=0x%p "
                "inode_free_list->num_free_inodeks = %d",
                __func__, free_list->first_node, free_list->num_free_inodes);
        return -ENOSPC;
    }
    ...
    while (temp) {
        ...
        LOG_FS("%s: step %d, curr_inodes %d, num_inodes %d\n", __func__, step,
               curr_inodes, num_inodes);
        if (num_inodes >= curr_inodes) {
            ...
            if (curr == free_list->first_node) {
                ...
                if (next_node) {
                    next =
                        container_of(next_node, struct sufs_range_node, node);
                }
            ...
            if (curr == free_list->last_node) {
                ...
                if (prev_node) {
                    prev =
                        container_of(prev_node, struct sufs_range_node, node);
                }
    ...
    if (free_list->num_free_inodes < num_inodes) {
        WARN_FS("%s: free list has %d free inodes, "
    ...
    if (found == 1) {
        ...
        WARN_FS("%s: Can't alloc.  found = %d", __func__, found);
        return -ENOSPC;
    }
...
int sufs_new_inodes(struct sufs_super_block *sb, int *inode_nr, int num_inodes,
                    int zero, int cpu) {
    struct sufs_inode_free_list *free_list = NULL;
    int new_inode_nr = 0;
    int ret_inodes = 0;
    ...
    if (num_inodes == 0) {
        WARN_FS("%s: num_inodes == 0", __func__);
        return -EINVAL;
    }
...
retry:
    ...
    if (not_enough_inodes(free_list, num_inodes)) {
        WARN_FS("%s: cpu %d, free_inodes %d, required %d, "
                "inode_node %d\n",
                __func__, cpu, free_list->num_free_inodes, num_inodes,
                free_list->num_inode_node);
...
alloc:
    LOG_FS("%s: cpu %d, free_inodes %d, required %d, "
            "inode_node %d\n",
            __func__, cpu, free_list->num_free_inodes, num_inodes,
            free_list->num_inode_node);
    ...
    LOG_FS("alloc inode %d, num_inodes %d, ret_inodes %d\n", new_inode_nr,
           num_inodes, ret_inodes);

    spin_unlock(&free_list->lock);

    if (ret_inodes <= 0 || new_inode_nr == 0) {
        WARN_FS("%s: not able to allocate %d inodes. "
...
int sufs_alloc_inode_to_libfs(unsigned long uaddr) {
    ...
    ret = sufs_new_inodes(&sufs_sb, &inode_nr, entry.num, 0, cpu);

    if (ret < 0)
        return ret;

    entry.num = ret;
    entry.inode_bm_lba = sufs_sb.inode_bitmap_start + inode_nr * FS_BLOCK_SIZE;
    entry.inode = inode_nr * FS_BLOCK_SIZE * 8;
...
int sufs_free_inode_from_libfs(unsigned long uaddr) {
    ...
    return sufs_free_inodes(
        &sufs_sb, (entry.inode - sufs_sb.inode_bitmap_start) / FS_BLOCK_SIZE,
        entry.num);
}

int sufs_kfs_set_inode(int ino, char type, unsigned int mode, unsigned int uid,
                       unsigned int gid, unsigned long index_offset) {
    int ret;
    unsigned long dma_buffer_va = sufs_kfs_dma_buffer_acquire(1, -1);
    if (!dma_buffer_va) {
        WARN_FS("Cannot acquire dma buffer\n");
        return -ENOMEM;
    }
    struct sufs_shadow_inode *si = sufs_find_sinode(ino, dma_buffer_va);

    if (si == NULL) {
        WARN_FS("Cannot find the shadow indode for inode : %d\n", ino);
        return -EFAULT;
    }
...
(unsigned long )si, (unsigned long) (&si->lease));
    ...
    ret = sufs_wb_sinode(ino, dma_buffer_va);
    sufs_kfs_dma_buffer_release(dma_buffer_va, 1);
    return ret;
}

struct sufs_shadow_inode *sufs_find_sinode(unsigned int ino_num,
    ...
    base_offset =
        ino_num * sizeof(struct sufs_shadow_inode) + sufs_sb.sinode_start;

    int ret;

    memset((void *)dma_buffer_va, 0, SUFS_PAGE_SIZE);
    ret = sufs_kfs_send_nvme_read(base_offset, FS_BLOCK_SIZE,
                                  dma_buffer_va_to_da(dma_buffer_va));
    if (ret < 0) {
        WARN_FS("Read inode from device failed\n");
        return NULL;
    }
    return (struct sufs_shadow_inode *)(dma_buffer_va);
}

int sufs_wb_sinode(unsigned int ino_num, unsigned long dma_buffer_va) {
    unsigned long base_offset;
    if (ino_num < 0 || ino_num > SUFS_MAX_INODE_NUM)
        return -EINVAL;
    ...
    return sufs_kfs_send_nvme_write(base_offset, FS_BLOCK_SIZE,
                                    dma_buffer_va_to_da(dma_buffer_va));
}