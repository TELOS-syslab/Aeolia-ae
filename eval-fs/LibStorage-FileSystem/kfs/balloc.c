...
int sufs_alloc_block_free_lists(struct sufs_super_block *sb) {
    ...
    sb->free_lists = kcalloc(cpus, sizeof(struct sufs_free_list), GFP_KERNEL);
    LOG_FS("cpus: %d\n", cpus);
    ...
    for (i = 0; i < cpus; i++) {
        free_list = sufs_get_free_list(sb, i);
...
static void sufs_init_free_list(struct sufs_super_block *sb,
    ...
    unsigned long per_list_block_pages = 0;

    int cpus = sb->cpus;

    per_list_block_pages = FS_FREE_BITMAP_LOGICAL_BLOCKS / cpus;

    free_list->block_start = per_list_block_pages * cpu;
    free_list->block_end = free_list->block_start + per_list_block_pages - 1;
}
...
void sufs_init_block_free_list(struct sufs_super_block *sb, int recovery) {
    ...
    for (i = 0; i < cpus; i++) {
        ...
        sufs_init_free_list(sb, free_list, i);
...
static void sufs_block_to_cpu_pm_nodes(struct sufs_super_block *sb, int blocknr,
    ...
    cpu_tmp = blocknr / per_list_block_pages;
...
static int sufs_find_free_slot(struct rb_root *tree, unsigned long range_low,
    ...
    if (ret) {
        WARN_FS("%s ERROR: %lu - %lu already in free list\n", __func__,
    ...
    if (!ret_node) {
        ...
        *next = ret_node;
        ...
        WARN_FS("%s ERROR: %lu - %lu overlaps with existing "
                "node %lu - %lu\n",
...
int sufs_free_blocks(struct sufs_super_block *sb, unsigned long blocknr,
    ...
    if (num_blocks <= 0) {
        WARN_FS("%s ERROR: free %lu\n", __func__, num_blocks);
        return -EINVAL;
    }

    sufs_block_to_cpu_pm_nodes(sb, blocknr, &cpuid);
    ...
    free_list = sufs_get_free_list(sb, cpuid);
    ...
    if (blocknr < free_list->block_start ||
        ...
        WARN_FS("free blocks %lx to %lx, free list addr: %lx, "
                "start %lu, end %lu\n",
                blocknr, blocknr + num_blocks - 1, (unsigned long)free_list,
...
static int not_enough_blocks(struct sufs_free_list *free_list,
    ...
    if (!first || !last ||
        ...
        WARN_FS("%s: num_free_blocks=%ld; num_blocks=%ld; "
                "first=0x%p; last=0x%p\n",
                __func__, free_list->num_free_blocks, num_blocks, first, last);
        return 1;
    }
...
static long sufs_alloc_blocks_in_free_list(struct sufs_super_block *sb,
    ...
    if (!free_list->first_node || free_list->num_free_blocks == 0) {
        WARN_FS("%s: Can't alloc. free_list->first_node=0x%p "
    ...
    if (free_list->num_free_blocks < num_blocks) {
        WARN_FS("%s: free list has %lu free blocks, "
...
int sufs_new_blocks(struct sufs_super_block *sb, unsigned long *blocknr,
    ...
    struct sufs_free_list *free_list = NULL;
    unsigned long new_blocknr = (unsigned long)-1;
    long ret_blocks = 0;
    int retried = 0;

    if (num_blocks == 0) {
        WARN_FS("%s: num_blocks == 0\n", __func__);
        return -EINVAL;
    }

retry:
    free_list = sufs_get_free_list(sb, cpu);
    if (!free_list) {
        WARN_FS("%s: free_list is NULL\n", __func__);
        return -EINVAL;
    }
    spin_lock(&free_list->s_lock);

    if (not_enough_blocks(free_list, num_blocks)) {
        WARN_FS("%s: cpu %d, free_blocks %lu, required %lu, "
                "blocknode %lu\n",
                __func__, cpu, free_list->num_free_blocks, num_blocks,
                free_list->num_blocknode);
...
alloc:
    ret_blocks =
        sufs_alloc_blocks_in_free_list(sb, free_list, num_blocks, &new_blocknr);

    spin_unlock(&free_list->s_lock);

    if (ret_blocks <= 0 || new_blocknr == (unsigned long)-1) {
        WARN_FS("%s: not able to allocate %lu blocks. "
    ...
    if (blocknr)
        *blocknr = new_blocknr;
    LOG_FS("[kfs] sufs_new_blocks: blocknr: %lu, ret_blocks: %ld\n", *blocknr,
           ret_blocks);
    return ret_blocks;
}
...
int sufs_alloc_blocks_to_libfs(unsigned long uaddr) {
    ...
    entry.num = ret;
    entry.bbm_lba = sufs_sb.block_bitmap_start + block_nr * FS_BLOCK_SIZE;
    entry.st_block =
        FS_HEAD_BYTES / SUFS_PAGE_SIZE + block_nr * FS_BLOCK_SIZE * 8;
...
int sufs_free_blocks_from_libfs(unsigned long uaddr) {
    ...
    return sufs_free_blocks(
        &sufs_sb, (entry.bbm_lba - sufs_sb.block_bitmap_start) / FS_BLOCK_SIZE,
        entry.num);
}
