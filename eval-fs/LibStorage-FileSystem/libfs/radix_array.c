...
static void
sufs_libfs_radix_array_init_upper_node(struct sufs_libfs_radix_array *ra,
...
unsigned long sufs_libfs_radix_array_find(struct sufs_libfs_radix_array *ra,
    ...
    if (ra->root_ == 0) {
        sufs_libfs_radix_array_init_upper_node(
            ra, (struct sufs_libfs_ra_node **)&ra->root_);
    }
    ...
    for (node_level = ra->levels; node_level > 0; --node_level) {
        ...
        if (sufs_libfs_ra_get_type(&next) == SUFS_LIBFS_RA_TYPE_NONE) {
            if (!fill) {
                ...
                sufs_libfs_radix_array_init_upper_node(
                    ra, (struct sufs_libfs_ra_node **)&unode->child[subkey]);
