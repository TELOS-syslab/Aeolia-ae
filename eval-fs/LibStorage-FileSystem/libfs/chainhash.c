...
const long sufs_libfs_hash_min_size = sufs_libfs_hash_sizes[0];
 ...
 */
static inline void sufs_libfs_ch_item_init(struct sufs_libfs_ch_item *item,
...
void sufs_libfs_chainhash_init(struct sufs_libfs_chainhash *hash, int index) {
    ...
    hash->buckets_ =
        malloc(hash->nbuckets_ * sizeof(struct sufs_libfs_ch_bucket));
...
sufs_libfs_get_buckets(struct sufs_libfs_chainhash *hash, char *key,
                       int max_size) {
    int bseq = 0, eseq = 0;
    struct sufs_libfs_ch_bucket *buckets = NULL;
    u64 nbuckets = 0;
...
bool sufs_libfs_chainhash_lookup(struct sufs_libfs_chainhash *hash, char *key,
                                 int max_size, unsigned long *vptr,
                                 unsigned long *vptr2) {
...
static void sufs_libfs_chainhash_resize(struct sufs_libfs_chainhash *hash,
    ...
    hash->buckets_resize_ =
        malloc(hash->nbuckets_resize_ * sizeof(struct sufs_libfs_ch_bucket));
...
bool sufs_libfs_chainhash_insert(struct sufs_libfs_chainhash *hash, char *key,
                                 int max_size, unsigned long val,
                                 unsigned long val2,
...
bool sufs_libfs_chainhash_replace_from(struct sufs_libfs_chainhash *dst,
...
__ssize_t sufs_libfs_chainhash_getdents(struct sufs_libfs_chainhash *hash,
                                        int max_size, unsigned long *offset_ptr,
    ...
    for (; i < hash->nbuckets_; i++) {
        ...
        for (item = b->head; item != NULL; item = item->next) {
            if (j > 0) {
                ...
                iter_ptr = (struct linux_dirent64 *)((unsigned long)buffer +
                                                     iter_offset);
            }
