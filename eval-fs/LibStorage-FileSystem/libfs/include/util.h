...
#define PAGE_ROUND_DOWN(a) ((((unsigned long)(a)) & ~(SUFS_PAGE_SIZE - 1)))

#define PAGE_OFFSET(a) (((unsigned long)(a)) & ((1 << SUFS_PAGE_SHIFT) - 1))

#define CACHE_ROUND_UP(a)                                                      \
    ((((unsigned long)(a)) + SUFS_CACHELINE - 1) & ~(SUFS_CACHELINE - 1))

#define CACHE_ROUND_DOWN(a) ((((unsigned long)(a)) & ~(SUFS_CACHELINE - 1)))

#define FILE_BLOCK_ROUND_UP(a)                                                 \
    ((((unsigned long)(a)) + SUFS_FILE_BLOCK_SIZE - 1) &                       \
     ~(SUFS_FILE_BLOCK_SIZE - 1))
...
static inline int sufs_libfs_getcpu(unsigned int *cpu, unsigned int *node) {
