...
static inline bool
sufs_libfs_chainhash_killed(struct sufs_libfs_chainhash *hash) {
    return hash->dead_;
}

static inline void
sufs_libfs_chainhash_fini(struct sufs_libfs_chainhash *hash) {
    if (hash->buckets_)
        free(hash->buckets_);
...
bool sufs_libfs_chainhash_lookup(struct sufs_libfs_chainhash *hash, char *key,
                                 int max_size, unsigned long *vptr1,
                                 unsigned long *vptr2);
