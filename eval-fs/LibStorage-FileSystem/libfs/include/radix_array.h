...
static inline size_t sufs_libfs_log2_exact(size_t x, size_t accum) {
    return (x == 0)         ? (1 / x)
           : (x == 1)       ? accum
           : ((x & 1) == 0) ? sufs_libfs_log2_exact(x >> 1, accum + 1)
                            : ~0;
}
...
sufs_libfs_ra_num_levels(struct sufs_libfs_radix_array *ra, unsigned level) {
    return sufs_libfs_ra_level_span(ra, level) >= ra->n
               ? level
               : sufs_libfs_ra_num_levels(ra, level + 1);
}
...
static inline unsigned int
sufs_libfs_ra_subkey(struct sufs_libfs_radix_array *ra, size_t k,
                     unsigned level) {
...
static inline unsigned int
sufs_libfs_ra_get_type(struct sufs_libfs_ra_node_ptr *ptr) {
    return (ptr->v & SUFS_LIBFS_RA_MASK);
}

static inline void
sufs_libfs_free_radix_array(struct sufs_libfs_radix_array *ra) {
    free((void *)ra->root_);
}
...
unsigned long sufs_libfs_radix_array_find(struct sufs_libfs_radix_array *ra,
                                          size_t index, int fill,
                                          unsigned long ps);
