...
# SPDK_SRC_DIR = os.environ['SPDK_SRC_DIR']
CUR_DIR = os.path.abspath(os.path.dirname(__file__))
BENCH_PERMIT_CPUS = 64
...
def setup_spdk(numa_node_num=None):
    setup_bin = '{}/{}'.format(SPDK_SRC_DIR,
                               'scripts/setup.sh')
    for no in [2, 3]:
        ret = subprocess.call(
            'HUGEMEM=16384 HUGENODE={} {}'.format(
                 no, setup_bin), shell=True)
        if ret != 0:
            print('cannot pin mem into node {}'.format(no))
    ret = subprocess.call(
        'echo 0 | sudo tee /proc/sys/kernel/randomize_va_space', shell=True)
    if ret == 0:
        print('ASLR (address space layout randomization disabled')
    print('setup spdk done')
...
class Runner(object):
    ...
    def __init__(self, \
        ...
        self.DIRECTIOS     = ["bufferedio"]  # enable directio except tmpfs -> nodirectio 
...
#        self.FS_TYPES      = [
        ...
        self.FXMARK_NAME_UFS = "fxmark-fsp"
        ...
        self.fxmark_ufs_path = os.path.normpath(
            os.path.join(CUR_DIR, self.FXMARK_NAME_UFS))
    ...
    def get_ncores(self):
        ...
        for n in test_hw_thr_cnts:
            if n > BENCH_PERMIT_CPUS:
                break
            ncores.append(n)
        return ncores

    def exec_cmd(self, cmd, out=None):
        print("[*********WILL EXEC : %s************]" % cmd)
    ...
    def umount(self, where):
        while True:
            if not self.init_nvme_disk():
                self.exec_cmd(f"{SPDK_SRC_DIR}/scripts/setup.sh reset")
            else:
    ...
    def mount_ufs(self, media, fs, mnt_path):
        setup_spdk()
        self.exec_cmd(MKFS_SPDK_BIN + " mkfs", self.dev_null)
        return True
    ...
    def get_bin_type(self, bench, fs):
        ...
        if fs == "ufs":
            return (self.fxmark_ufs_path, bench)
    ...
    def fxmark(self, media, fs, bench, ncore, nfg, nbg, dio):
        ...
        (bin, type) = self.get_bin_type(bench, fs)
        ...
        cmd = ' '.join(["env", 
                        ...
                        "--directio", directio,
                        "--root", self.test_root if fs != "ufs" else "/ufs_root",
    ...
    def run(self):
        try:
            ...
            self.log_start()
            total = 0
            for (media, fs, bench, ncore, dio) in self.gen_config():
                total += 1
            
            with tqdm(total=total, unit="test") as pbar:
                for (cnt, (media, fs, bench, ncore, dio)) in enumerate(self.gen_config()):
                    ...
                    self.pre_work()
                    if(fs == "ufs"):
                        fsp_proc = start_fsp(nfg, 10, self.dev_null)

                    pbar.set_description("Running %s:%s:%s:%s:%s" % (media, fs, bench, nfg, dio))
                    self.fxmark(media, fs, bench, ncore, nfg, nbg, dio)
                    if(fs == "ufs"):
                        shutdown_fsp(fsp_proc)
                    self.post_work()
